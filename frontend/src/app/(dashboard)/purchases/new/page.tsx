...
export default function CreatePurchasePage() {
  ...
  const [supplierData, setSupplierData] = useState({ name: "", email: "", phone: "", type: "SUPPLIER" });
  ...
  const createPurchase = useMutation({
    mutationFn: (newPurchase: any) => apiClient.post("/purchases/invoices", newPurchase),
    onSuccess: () => {
  ...
  const createParty = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setSupplierData({ name: "", email: "", phone: "", type: "SUPPLIER" });
    },
  ...
  const handleCreateSupplier = (e: React.FormEvent) => {
    e.preventDefault();
    createParty.mutate(supplierData);
  };
  ...
  const handleSave = (e: React.FormEvent, status: "PENDING" | "RECEIVED" = "PENDING") => {
    e.preventDefault();
    ...
    <div className="flex flex-col gap-6 w-full max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Create a new purchase bill</p>
        </div>
      </div>
      ...
      <Dialog open={isSupplierOpen} onOpenChange={setIsSupplierOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleCreateSupplier} className="space-y-4">
            <div className="space-y-2"><Label>Name *</Label><Input required value={supplierData.name} onChange={e => setSupplierData({...supplierData, name: e.target.value})} /></div>
            <div className="space-y-2"><Label>Email</Label><Input type="email" value={supplierData.email} onChange={e => setSupplierData({...supplierData, email: e.target.value})} /></div>
            <DialogFooter><Button type="button" variant="outline" onClick={() => setIsSupplierOpen(false)}>Cancel</Button><Button type="submit" disabled={createParty.isPending}>Save</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <form className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant="outline" size="icon" onClick={() => setIsSupplierOpen(true)}><Plus className="h-4 w-4" /></Button>
                </div>
              </div>
        ...
        <div className="flex items-center justify-end gap-4 pt-4">
          <Button variant="outline" type="button" onClick={(e) => handleSave(e, "PENDING")} disabled={createPurchase.isPending}><Save className="mr-2 h-4 w-4" /> Save as Pending</Button>
          <Button type="button" onClick={(e) => handleSave(e, "RECEIVED")} disabled={createPurchase.isPending}>{createPurchase.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />} Save as Received</Button>
        </div>
      </form>
    </div>
  );
